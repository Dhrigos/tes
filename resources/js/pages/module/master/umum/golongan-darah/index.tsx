...
useEffect(() => {
  ...
  const filteredGoldars = goldars.filter((g) =>
    `${g.nama} ${g.rhesus}`.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editId ? "Edit Golongan Darah" : "Tambah Golongan Darah"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            <Input
              placeholder="Nama Golongan Darah"
              value={nama}
