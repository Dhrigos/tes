...
export default function Index() {
  ...
  // Computed Values
  const filteredSpesialis = spesialis.filter((a) =>
    ...
    <AppLayout breadcrumbs={breadcrumbs}>
        ...
        <Card className="border-border bg-card">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <RefreshCw className={`w-4 h-4 mr-2 ${syncSpesialisLoading ? "animate-spin" : ""}`} />
                Sinkron Spesialis
              </Button>
            </div>
          </CardHeader>
                  ...
                  filteredSpesialis.map((item, index) => (
                    <React.Fragment key={item.id}>
                      ...
                      <TableRow key={item.id} className="border-border bg-muted/20 hover:bg-muted/40">
                        ...
                        >
                        <RefreshCw
                            className={`w-4 h-4 ${syncingSubId === item.kode ? "animate-spin" : ""}`}
                        />
                        </Button>
                          <Button
                            size="sm"
        ...
        <DialogContent className="bg-background border-border">
          <DialogHeader>
            <DialogTitle className="text-foreground">
              {editId ? "Edit Spesialis" : "Tambah Spesialis"}
            </DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={subOpen} onOpenChange={setSubOpen}>
        <DialogContent className="bg-background border-border">
          <DialogHeader>
            <DialogTitle className="text-foreground">
              {subEditId ? "Edit Sub Spesialis" : "Tambah Sub Spesialis"}
            </DialogTitle>
          </DialogHeader>
            ...
            >
              ...
              <SelectContent className="bg-background border-border">
                {spesialis.map((spesialisItem) => (
                  <SelectItem
                    key={spesialisItem.id}
                    value={spesialisItem.id.toString()}
      ...
      <Dialog open={syncSpesialisOpen} onOpenChange={setSyncSpesialisOpen}>
        <DialogContent className="bg-background border-border">
          <DialogHeader>
            <DialogTitle className="text-foreground">
              Konfirmasi Sinkronisasi Spesialis
            </DialogTitle>
          </DialogHeader>
          <p className="text-foreground">
            Apakah Anda yakin ingin melakukan sinkronisasi data spesialis dari sumber eksternal?
          </p>
          <DialogFooter>
            <Button
              type="button"
            ...
            <Button
              ...
              disabled={syncSpesialisLoading}
            >
              {syncSpesialisLoading && <RefreshCw className="w-4 h-4 mr-2 animate-spin" />}
              Sinkronkan
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={syncSubspesialisOpen} onOpenChange={setSyncSubspesialisOpen}>
        <DialogContent className="bg-background border-border">
          <DialogHeader>
            <DialogTitle className="text-foreground">
              Konfirmasi Sinkronisasi Subspesialis
            </DialogTitle>
          </DialogHeader>
          <p className="text-foreground">
            Apakah Anda yakin ingin melakukan sinkronisasi data subspesialis dari sumber eksternal?
          </p>
          <DialogFooter>
            <Button
              type="button"
