...
export default function Index() {
    ...
    // Computed Values
    const filteredJenises = jenises.filter((jenis) => jenis.nama.toLowerCase().includes(search.toLowerCase()));
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
                                        ...
                                        filteredJenises.map((jenis, index) => (
                                            <>
                                                ...
                                                <TableRow key={jenis.id} className="hover:bg-muted/50">
                                                    ...
                                                    <TableCell className="px-6 py-4 font-medium">{jenis.nama}</TableCell>

                                                    <TableCell className="items-end px-6 py-4">
                                                        <div className="flex justify-end space-x-2">
                                                            ...
                                                            <Button size="sm" variant="destructive" onClick={() => handleOpenDelete(jenis, 'jenis')}>
                                                ...
                                                {/* Label Pemeriksaan */}
                                                {expandedRows.has(jenis.id) && (
                                                    <TableRow key={`label-${jenis.id}`} className="bg-muted/10">
                                                ...
                                                {expandedRows.has(jenis.id) &&
                                                    jenis.pemeriksaans &&
                                                    jenis.pemeriksaans.length > 0 &&
                                                    jenis.pemeriksaans.map((pemeriksaan, pemeriksaanIndex) => (
                                                        <TableRow key={`pemeriksaan-${pemeriksaan.id}`} className="hover:bg-muted/20">
                                                            <TableCell className="px-6 py-4">
                                                                <div className="flex items-center gap-2 pl-8">{pemeriksaanIndex + 1}</div>
                                                            </TableCell>

                                                            <TableCell className="px-6 py-4 text-muted-foreground">{pemeriksaan.nama}</TableCell>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader className="pb-4">
                        <DialogTitle className="text-lg">{editId ? 'Edit Jenis Radiologi' : 'Tambah Jenis Radiologi'}</DialogTitle>
                    </DialogHeader>
            ...
            <Dialog open={pemeriksaanOpen} onOpenChange={setPemeriksaanOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader className="pb-4">
                        <DialogTitle className="text-lg">{pemeriksaanEditId ? 'Edit Pemeriksaan' : 'Tambah Pemeriksaan'}</DialogTitle>
                    </DialogHeader>
                    ...
                    <form onSubmit={handlePemeriksaanSubmit} className="space-y-4">
                        ...
                        <DialogFooter className="flex justify-end space-x-3 pt-4">
                            <Button type="button" variant="outline" onClick={() => setPemeriksaanOpen(false)}>
                                Batal
                            </Button>
                            <Button type="submit" className="min-w-[100px]">
            ...
            <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
                <DialogContent className="sm:max-w-md">
                    ...
                    <div className="py-4">
                        ...
                        {deleteType === 'jenis' && <p className="mt-2 text-sm text-muted-foreground">Semua pemeriksaan terkait juga akan dihapus.</p>}
                    </div>
