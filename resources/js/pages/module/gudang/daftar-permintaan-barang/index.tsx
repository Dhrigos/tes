...
export default function DaftarPermintaanBarangIndex({ title, permintaan, dabar, auth }: Props) {
    ...
    const [manualItems, setManualItems] = useState<
        Array<{ kode_barang: string; nama_barang: string; jumlah: number; jenis_barang?: string; harga_dasar?: number }>
    >([]);
    ...
    const moveToRequestForm = async () => {
        ...
        // Fetch harga dasar for each selected item
        const itemsWithHargaDasar = [];
        for (const item of selectedItems) {
            try {
                const hargaResponse = await axios.get(`/api/daftar-permintaan-barang/get-harga-dasar/${item.kode_barang}`);
                const hargaDasar = hargaResponse.data.success ? hargaResponse.data.harga_dasar : 0;

                itemsWithHargaDasar.push({
                    ...item,
                    harga_dasar: hargaDasar,
                });
            } catch (error) {
                console.error(`Error fetching harga dasar for ${item.kode_barang}:`, error);
                // Add item without harga dasar if fetch fails
                itemsWithHargaDasar.push({
                    ...item,
                    harga_dasar: 0,
                });
        ...
        itemsWithHargaDasar.forEach((item) => {
            // Check if item already exists in manualItems
            const exists = newManualItems.some((manualItem) => manualItem.kode_barang === item.kode_barang);
    ...
    const addObatManual = () => {
        ...
        if (existingItemIndex !== -1) {
            ...
            const newItem = {
                ...
                harga_dasar: 0, // Default harga_dasar to 0 for manually added items
            };
        ...
        try {
            ...
            const requestData = {
                ...
                items: manualItems.map((item) => ({
                    ...
                    harga_dasar: item.harga_dasar?.toString() || '0',
                })),
            };
            ...
            const response = await axios.post('/api/daftar-permintaan-barang/proses-permintaan', requestData, {
                headers: {
    ...
    const handleSelectRow = (kodeBarang: string) => {
        setSelectedRows((prev) => {
            const newSelected = prev.includes(kodeBarang) ? prev.filter((id) => id !== kodeBarang) : [...prev, kodeBarang];
